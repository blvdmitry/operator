...
const trapFocus = (() => {
  ...
  let srTrap: ReturnType<typeof screenReaderTrap> | null = null;
  ...
  return (root: HTMLElement, options: TrapOptions = {}): Trap => {
    const {
      ...
      navigationMode = "tab",
      onNavigateOutside,
      onReleaseFunctionUpdate,
      releaseFocusTargetTrigger,
    } = options;
    let queueId: number;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (hasNavigatedOutside) {
         ...
         */
        if (!isFocusedOnTrigger) event.preventDefault();
    ...
    const init = () => {
      ...
      if (trapMode === "hard") srTrap = screenReaderTrap(root);
