...
const useCalendarDay = (date: Date) => {
  const {
    setHoveredDateISO,
    hoveredDateISO,
    startDateISO,
  ...
  const isInHoverRange =
    startDateISO && !endDateISO && !singleDate && dateISO < hoveredDateISO!;
  ...
  const handleMouseOut = () => isHovered && setHoveredDateISO(null);

  const handleClick = () => {
    if (isDisabled) return;

    const nextState: Pick<T.DateChangeArgs, "startDate" | "endDate"> = {
      startDate,
      endDate,
    };
