...
const InputSliderTooltip = (props: T.ControlProps) => {
  ...
  const tooltipWrapperRef = React.useRef<HTMLSpanElement | null>(null);
  const tooltipClassName = classNames(
    styles["handle-tooltip"],
    !!mergedTooltipDelta && styles["handle-tooltip--hidden-content"]
  );
  const OFFSET = 14;
  ...
  const renderContent = (value: number) => {
    if (renderTooltipValue) return renderTooltipValue(value);
    return value;
  };
  ...
  return (
    <>
      <span className={tooltipClassName} ref={tooltipWrapperRef}>
        <span
          ref={tooltipRef}
          className={styles["handle-tooltip-content"]}
          style={{ transform: `translateX(${deltaX}px)` }}
        >
          {renderContent(controlValue)}
        </span>
      </span>

      {hasMergedTooltip && (
        <span className={styles["handle-tooltip"]} ref={mergedTooltipRef}>
          <span
            className={styles["handle-tooltip-content"]}
            style={{ transform: `translateX(${mergedDeltaX}px)` }}
          >
            {renderContent(minValue)} - {renderContent(maxValue)}
          </span>
        </span>
      )}
    </>
...
const InputSliderControl = (
  ...
  const handleClassName = classNames(
    styles.handle,
    (valueVisibility === "tooltip" || dragId === id) &&
      styles["handle--visible"],
    dragId === id && styles["handle--active"]
  );
  const controlValue = id === minId ? minValue : maxValue;
  ...
  return (
    <>
      ...
      <div
        ...
        ref={ref}
        style={{ [direction]: `${percentValue * 100}%` }}
        onMouseDown={() => onDragStart(id)}
        onTouchStart={() => onDragStart(id)}
        role="none"
      >
        {valueVisibility !== "hidden" && <InputSliderTooltip {...props} />}
      </div>
    </>
  );
};
