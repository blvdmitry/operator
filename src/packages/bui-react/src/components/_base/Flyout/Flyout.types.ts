...
import type {
  TrapMode,
  NavigationMode,
} from "@bookingcom/bui-core/utilities/a11y";
import type {
  FlyoutPosition,
  FlyoutPositionCompensation,
...
type PassedFlyoutOptions = {
  ...
  ) => FlyoutPositionCompensation | undefined;
};

export type FlyoutOptions = Pick<
  PassedFlyoutOptions,
  "forcePosition" | "zIndex"
> & {
  position: FlyoutPosition;
  rtl: boolean;
...
type FlyoutMoveAction = {
  type: "move";
  payload: { x: number; y: number; isRTL: boolean };
};
...
export type UseFlyout = (
  originRef: ElementRef,
  targetRef: ElementRef,
  options: PassedFlyoutOptions
) => Pick<FlyoutState, "styles" | "position" | "active" | "visible"> & {
  render: () => void;
  show: () => void;
  hide: () => void;
  remove: () => void;
...
export type TriggerEventSetListeners = (
  el: HTMLElement,
  handlers: Partial<Record<keyof GlobalEventHandlersEventMap, (e: any) => void>>
) => undefined | (() => void);

export type TriggerEventSetAttributes = (
  el: HTMLElement,
  attributes: TriggerAttributesHtml
) => () => void;
